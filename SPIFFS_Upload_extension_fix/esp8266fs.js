...
function getEspPackagePath(arduinoUserPath, preferencesPath, target) {
    switch (target.architecture) {
        ...
        case "esp32": {
            const esp32Path = path.join(arduinoUserPath, "packages", target.package, "hardware", target.architecture);
            console.log(esp32Path);
...
function getSpiffsPartition(packagesPath, partition) {
    var data = {};
    readLines(path.join(packagesPath, "1.0.4", "tools", "partitions", partition + ".csv"))
        .forEach(line => {
...
function getSpiffsOptions(packagesPath, target, arduinoJson, preferences) {
    const spiffsOptions = {};
    readLines(path.join(packagesPath, "1.0.4", "boards.txt"))
        .forEach(line => {
...
function getEspToolsPath(arduinoUserPath, preferencesPath, target) {
    const dir = target.architecture == "esp8266"
                ...
                : path.resolve(path.join(arduinoUserPath, "packages", target.package, "hardware", target.architecture, "1.0.4", "tools"));
...
function _uploadSpiffsEspToolPy(esptool, commPort, spiffsImage, spiffsOptions, target) {
    ...
    const flashFreq = target.flashFreq+"m";
    const flashSize = target.flashSize+"B" || "detect";
